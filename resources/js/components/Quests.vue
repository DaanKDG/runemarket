          ...
          <table class="table" style="width: 90% !important;">
            ...
            <tbody>
              <tr v-for="quest in quests" v-bind:key="quest.id">
                ...
                <td
                  style="color: #09814A; font-weight: bold;"
                >{{quest.price ? quest.price + 'M' : 'no price set'}}</td>
                <td>
                  <a @click="addQuest(quest)" href="#" class="btn btn-outline-dark">Add</a>
                </td>
              </tr>
            </tbody>
          </table>
            ...
            <div class="card-body">
              ...
              <ul class="list-group list-group-flush" v-for="quest in order" v-bind:key="quest.id">
                <li class="list-group-item" style="padding-left: 0;">
                  {{quest.name}}
                  <i @click="removeQuest(quest)" class="fas fa-trash ml-1"></i>
                  <span
                    style="color: #09814A; font-weight: bold;"
                    class="float-right"
                  >{{quest.price ? quest.price : 0}}M</span>
                </li>
              ...
              <div class="total-price mt-3">
                <p class="float-right" style="font-weight: bold; margin: 0;">
                  ...
                  <span
                    style="color: #09814a; padding-right: 1.25rem;"
                  >{{this.orderPrice ? this.orderPrice + 'M' : ''}}</span>
                </p>
              </div>
            </div>
          </div>
  ...
  methods: {
    ...
    sendOrderMsg() {
      ...
      this.order.forEach(order => {
        ...
        if (this.orderMsg) {
          this.orderMsg += "," + " " + order.name;
        } else {
          this.orderMsg = order.name;
        }
      });
      tidioChatApi.messageFromVisitor("Hey, I want to order the following quest(s): " + this.orderMsg  + ' ' + 'for a total price:' + ' ' + this.orderPrice + 'M');
    }
  }
};
</script>
